...
class OpeningRangeBreakoutParameters(BaseStrictModel):
    ...
    stop_model: Literal["opposite_range", "atr", "opposite_range_or_atr"] = Field("opposite_range_or_atr")
    ...
    def on_bar(self, ctx: StrategyContext, bar: Bar) -> list[OrderIntent]:
        ...
        if not self.state["opening_range_complete"]:
            ...
            if or_high is not None and or_low is not None:
                ...
                regular_bars = [b for b in bars if b.session_type == "regular" and b.session_date == bar.session_date]
                if regular_bars:
                    session_start_time = regular_bars[0].bar_start_utc
                    if bar.bar_end_utc.timestamp() >= session_start_time.timestamp() + (params.opening_range_minutes * 60):
                        or_width_pct = (or_high - or_low) / or_low * 100
                        if params.minimum_opening_range_pct <= or_width_pct <= params.maximum_opening_range_pct:
        ...
        if self.state["opening_range_complete"] != True:
            return []
            
        if local_time > cutoff_time:
            return []
            
        total_trades = self.state["trades_taken_by_direction"]["long"] + self.state["trades_taken_by_direction"]["short"]
        ...
        lookback = bars[-params.volume_ratio_lookback_bars - 1 : -1] if len(bars) > params.volume_ratio_lookback_bars else []
        ...
        is_long_valid = params.directions in ["long", "both"] and self.state["trades_taken_by_direction"]["long"] == 0
        is_short_valid = params.directions in ["short", "both"] and self.state["trades_taken_by_direction"]["short"] == 0
        ...
        if is_long_valid and bar.close > (or_high + buffer):
            ...
            if risk <= 0: return []
            target_price = entry_price + (risk * params.reward_risk)
        ...
        if signal_dir:
            ...
            # Since OrderIntent doesn't directly take `bracket` or `features`, we map it to limit_price, stop_price etc for now.
            # Wait, OrderIntent has limit_price and stop_price. The engine will convert it.
            # We are generating an intent of type "market". But the intent needs to know the stop and target.
            # Ah, wait! The strategy_specs says "valid bracket". But `OrderIntent` schema:
            # It only has limit_price and stop_price for limit and stop entries. How to specify bracket in OrderIntent?
            # Let's check `OrderIntent` in `src/edgeback/domain/orders.py`. It only has `limit_price` and `stop_price`, and doesn't specify take_profit.
            # But the spec says "stop" and "target". We will just use create_intent for the market order for entry, and wait for engine to fill it. 
            # Or the strategy should emit bracket intent?
            # Let's look at `OrderIntent`. It lacks bracket fields. Let's add them to OrderIntent in `src/edgeback/domain/orders.py`.
            # First, just append what we can.
            
            # For now, let's just make the entry intent.
            intent = ctx.create_intent(
                symbol=bar.symbol,
                direction=signal_dir,
                intent_type="market"
            )
            # Monkeypatching bracket on intent just in case, but it's not valid pydantic unless we add it
            # I will need to update OrderIntent.
