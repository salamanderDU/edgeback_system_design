...
def calculate_opening_range(bars: Sequence[Bar], opening_range_minutes: int) -> tuple[float | None, float | None]:
    """
    Calculate the high and low of the opening range.
    The opening range consists of all complete 'regular' session bars whose interval lies entirely 
    within the first `opening_range_minutes` of the regular session.
    """
    if not bars:
        return None, None
