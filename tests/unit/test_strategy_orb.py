...
def base_bars():
    ...
    for i in range(20):
        b = Bar(
            ...
            interval_seconds=300, session_type="pre", session_date=datetime.date(2023, 10, 2),
            is_complete=True, source_provider="test", source_feed="test",
            adjustment_mode="split_adjusted", ingested_at_utc=start_dt
        )
        bars.append(b)
    ...
    for i in range(3):
        b = Bar(
            ...
            open=100.0, high=102.0, low=100.0, close=101.0, volume=5000,
            interval_seconds=300, session_type="regular", session_date=datetime.date(2023, 10, 2),
            is_complete=True, source_provider="test", source_feed="test",
            adjustment_mode="split_adjusted", ingested_at_utc=start_dt
        )
        bars.append(b)
...
def test_orb_long_breakout(base_bars):
    ...
    breakout_bar = Bar(
        ...
        open=101.0, high=104.0, low=101.0, close=103.0, volume=10000, # higher volume
        interval_seconds=300, session_type="regular", session_date=datetime.date(2023, 10, 2),
        is_complete=True, source_provider="test", source_feed="test",
        adjustment_mode="split_adjusted", ingested_at_utc=start_dt
    )
    ...
    context = CausalStrategyContext(engine_time_utc=breakout_bar.bar_end_utc, bars_by_symbol={"AAPL": all_bars})
...
def test_orb_rejects_low_volume(base_bars):
    ...
    breakout_bar = Bar(
        ...
        bar_start_utc=start_dt + datetime.timedelta(minutes=15),
        bar_end_utc=start_dt + datetime.timedelta(minutes=20),
        open=101.0, high=104.0, low=101.0, close=103.0, volume=100, # low volume
        interval_seconds=300, session_type="regular", session_date=datetime.date(2023, 10, 2),
        is_complete=True, source_provider="test", source_feed="test",
        adjustment_mode="split_adjusted", ingested_at_utc=start_dt
    )
...
def test_orb_cutoff_time(base_bars):
    ...
    breakout_bar = Bar(
        ...
        open=101.0, high=104.0, low=101.0, close=103.0, volume=10000,
        interval_seconds=300, session_type="regular", session_date=datetime.date(2023, 10, 2),
        is_complete=True, source_provider="test", source_feed="test",
        adjustment_mode="split_adjusted", ingested_at_utc=start_dt
    )
